...
unsigned int end_segment_realtime = 0;
...
int write_index_file(const struct options_t options, const unsigned int first_segment, const unsigned int last_segment, const int end) {
	...
	if (options.key)
	{
		snprintf(write_buf, 1024, "#EXT-X-VERSION:3\n#EXT-X-KEY:METHOD=AES-128,URI=\"%s\",IV=0x%s\n", options.keyUrl, phexString((uint8_t*)options.iv));
		if (fwrite(write_buf, strlen(write_buf), 1, index_fp) != 1) {
        fprintf(stderr, "Could not write to m3u8 index file, will not continue writing to index file\n");
        free(write_buf);
        fclose(index_fp);
	...
	if (i == last_segment && end)
	{
		
        	snprintf(write_buf, 1024, "#EXTINF:%u,\n%s%s-%u.ts\n", end_segment_realtime, options.url_prefix, options.output_prefix, i);
	}
	else
	{
...
void display_usage(void)
{
    ...
    printf("segmenter example:\n./segmenter -i test.ts -d 10 -p seg -m seg.m3u8 -u ''");
    printf("aes segmenter example:\n./segmenter -i test.ts -d 10 -p seg -m seg.m3u8 -u '' -K \"http://10.130.29.36/self.key\"");
    printf("\n");
    printf("\n");
...
int main(int argc, char **argv)
{
	...
	if (options.keyUrl && ( (options.key == NULL) || (strlen(options.key) != 16)))
	{
		...
		phex((uint8_t*)options.iv);
	}
	else if (options.keyUrl)
	{
		int i;
		srand(time(NULL));
		FILE *fp;
    ...
    *dot = '.';
    ...
    do {
		...
		//	printf("AV_PKT_FLAG_KEY.segment_time=%d\n", segment_time);
        }
        else if (video_index < 0) {
            segment_time = packet.pts * av_q2d(audio_st->time_base);
		//	printf("else if .segment_time=%d\n", segment_time);
        }
        else {
		//	printf("else .segment_time=%d\n", segment_time);
          segment_time = prev_segment_time;
		  //printf("else .segment_time=%d", segment_time);
        }


//printf("if before segment_time-prev_segment_time=%lf, options.segment_duration=%ld\n", segment_time - prev_segment_time, options.segment_duration);
        end_segment_realtime = segment_time - prev_segment_time;
        if (segment_time - prev_segment_time >= options.segment_duration) {
			//printf("readPacketNum:%d\n", readPacketNum);
            av_write_trailer(oc);   // close ts file and free memory
            avio_flush(oc->pb);
			//printf("segment_time-prev_segment_time=%lf, options.segment_duration=%ld\n", segment_time - prev_segment_time, options.segment_duration);
            avio_close(oc->pb);
